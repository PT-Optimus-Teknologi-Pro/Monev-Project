    ...
    useEffect(() => {
        const filteringDataRab = () => {
            const dataFilter = realisasiData?.filter((item) => {
                ...
                const selectedPPK = item?.schedule?.rab!.data_entry.selected_ppk_id == user?.id;

                return tahunFilter && searchFilter && satuanKerjaFilter && selectedPPK;
            });
        ...
        filteringDataRab();
    }, [search, satuanKerja, tahun, realisasiData, user]);
