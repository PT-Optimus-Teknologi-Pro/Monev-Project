...
import { Search, ChevronDown, Calendar, Package, Sparkles, Building2 } from 'lucide-react';
...
export default function SearchData({ setSelectedRealization }: searchDataProps) {
    ...
    const transformedSatkerData: OptionProps[] = satkerData?.map((item: any) => ({
        text: item?.text ?? String(item),
    })) || [];

    const filteredRealisasiData = realisasiData.filter((item) =>
        item.schedule.rab?.data_entry.satuan_kerja?.toLowerCase() === satker.toLowerCase()
    )

    const paketCode: OptionProps[] = filteredRealisasiData.map((item: any) => ({
            ...
            <div className="relative group">
                ...
                <div className="relative">
                    <div className="absolute inset-y-0 left-0 pl-3 sm:pl-4 flex items-center pointer-events-none z-10">
                        <div className="p-1.5 rounded-lg bg-hover transition-all duration-300">
                            {icon}
                        </div>
                    </div>
                    <button
                        ...
                        className="w-full pl-14 sm:pl-16 pr-10 py-3.5 sm:py-4 border-2 border-gray-200 rounded-xl transition-all duration-300 focus:outline-none focus:border-third focus:ring-4 focus:ring-hover text-left bg-white hover:border-third hover:shadow-lg"
                    >
                        <span className={`font-poppins-medium text-sm sm:text-base truncate block ${selectedOption ? 'text-gray-800' : 'text-gray-400'}`}>
                            {isPaket ? (
                                selectedOption
                                    ...
                                    : filteredRealisasiData?.[filteredRealisasiData.length - 1]?.schedule.rab?.data_entry.nama_paket
                                        ? `${filteredRealisasiData[filteredRealisasiData.length - 1].schedule.rab?.data_entry.nama_paket} (${filteredRealisasiData[filteredRealisasiData.length - 1].schedule.rab?.data_entry.kode_paket})`
                                        : "Tidak Ada"
                            ) : (
                ...
                {isOpen && (
                    <div className="absolute z-30 w-full mt-2 bg-white border-2 border-hover rounded-xl shadow-2xl overflow-hidden animate-fade-in">
                        <div className="p-2.5 sm:p-3 bg-linear-to-r from-hover to-hover/50 border-b border-hover">
                            <div className="relative">
                            ...
                            {filteredOptions.length > 0 ? (
                                filteredOptions.map((option, index) => (
                                    <button
                                        ...
                                        className={`w-full font-poppins-regular text-left px-3 sm:px-4 py-2.5 sm:py-3 hover:bg-linear-to-r hover:from-hover hover:to-hover/50 transition-all duration-200 border-b border-gray-100 last:border-0 ${value === option.text ? 'bg-linear-to-r from-hover to-hover font-poppins-semibold border-l-4 border-l-third' : ''
                                    ...
                                    >
                                        {isPaket ? (
                                            <div className="flex flex-col">
                                                ...
                                                {option.label && <span className="text-xs text-gray-500 mt-0.5 ml-4">({option.text}) | {`Nama PPK: ${option.ppk}`}</span>}
                                            </div>
                                        ) : (
    ...
    return (
        <div
            ref={sectionRef}
            className="min-h-screen flex items-center bg-linear-to-br from-hover/30 via-white to-hover/20"
        >
            <div className="w-full max-w-5xl mx-auto px-4 sm:px-6 md:px-8 pt-24 sm:pt-28 pb-10 sm:pb-16">
                <div className={`text-center mb-6 sm:mb-8 transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
                    <div className="inline-flex items-center space-x-2 px-3 sm:px-4 py-1.5 sm:py-2 bg-linear-to-r from-hover to-hover rounded-full mb-3 sm:mb-4">
                        <Sparkles className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-third" />
                        <span className="text-xs sm:text-sm font-poppins-semibold text-secondary">Sistem Pencarian Data</span>
                    </div>
                    <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-poppins-bold mb-3 sm:mb-4 bg-linear-to-r from-secondary via-third to-secondary bg-clip-text text-transparent leading-tight px-2">
                    ...
                    <p className="text-gray-600 text-sm sm:text-base md:text-lg max-w-2xl mx-auto font-poppins-regular leading-relaxed px-2">
                        Cari dan lihat detail realisasi pekerjaan konstruksi berdasarkan tahun anggaran dan paket pekerjaan dengan mudah dan cepat
                    </p>
                </div>
                ...
                >
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 mb-6 sm:mb-8">
                        ...
                        {renderDropdown(
                            'satker', 'Satuan Kerja',
                            <Building2 className="h-4 w-4 sm:h-5 sm:w-5 text-third" />,
                            satker, setSatker,
                            searchSatker, setSearchSatker,
                            transformedSatkerData
                        )}
                        {renderDropdown(
                            'paket', 'Paket',
